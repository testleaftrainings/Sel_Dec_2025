...
4:45 to 4:55 ---> Break
5:00 to 5:45 ---> Frames
5:45 to 6:10 ---> BreakOut

Set:
-interface,not posibe to create an object so it needs an implementation class
implementation class:
1)HashSet-random order
2)LinkedHashSet-insertion order
3)TreeSet-Ascii order
-unordered collection bec it depends on the implementation classes.
-Set does not allows the duplicate values
-cannot have get() method.

int num[]={2,4,3,5,4}
index of 4--->using list(get)-->4
num[]={2,4,3,5}
index of 4--->exception
